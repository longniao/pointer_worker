...
class Rsi(db.Document):
    ...
    rsi = db.FloatField(required=True)
    ...
    meta = {'db_alias': 'market', 'collection': 'indicator_rsi', 'strict': False}
    ...
    def __repr__(self):
        return '<Indicator Rsi ex:\'%s\', contract:\'%s\'>' % (self.ex, self.contract)
    ...
    def insert_data(data):
        ...
        # 检测，防止重复插入
        indicator = Rsi.objects(ex=data['ex'], contract=data['contract'], freq=data['freq'], time=data['time']).first()

        if not indicator:
            return Rsi(**data).save()
        else:
            ...
            if new_time == old_time:
                update_data = dict()
                if 'rsi' in data and data['rsi'] != indicator.rsi:
                    update_data['rsi'] = data['rsi']
                if update_data:
                    print('indicator rsi update:', indicator._id, update_data)
                    update_data['utime'] = arrow.utcnow().datetime
                    Rsi.objects(_id=indicator._id).update_one(**update_data)
            return True
